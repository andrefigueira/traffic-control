...
It does not fully catch **semantic** coupling. One agent changing a function signature while another edits a caller in a different file will pass clearance and still break the build. There is now an opt-in first cut at this: set `TC_SYMBOLS=1` and the `PreToolUse` hook warns when the file you are about to edit and a file another agent holds share an exported symbol that one defines and the other uses. It is a heuristic (regex over source, not a parser, to keep the binary dependency-free), covers Go, TypeScript/JavaScript, and Python, compares only files of the same language, and is advisory, so it is awareness rather than a guarantee. The board and the scope remain the broader answer here.
